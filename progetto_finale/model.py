...
- Prima la generazione calcolava `output_head` su tutte le posizioni del
  contesto.
- Qui la generazione calcola `output_head` solo sull'ultimo token e può usare
  `scaled_dot_product_attention` come ottimizzazione opzionale.

Scopo del file:
- Fornire il `LanguageModel` finale del percorso didattico.
- Mantenere insieme Transformer blocks, dropout, weight tying e generazione.
- Lasciare la self-attention manuale come default didattico, con attention
  ottimizzata attivabile quando serve.
"""
...
class LanguageModel(nn.Module):
    ...
    def generate(self, input_ids, max_new_tokens, temperature=1.0, top_k=None):
        if input_ids.ndim != 2:
            raise ValueError("input_ids deve avere forma [batch_size, context_size].")

        if input_ids.shape[1] == 0:
            raise ValueError("input_ids deve contenere almeno un token.")
