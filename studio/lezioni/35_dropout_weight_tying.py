...
- Prima il modello non usava dropout e aveva pesi separati tra input embedding
  e output head.
- Qui verifichiamo dropout e weight tying sul modello della lezione 35.

Scopo del file:
- Mostrare che il dropout cambia il comportamento in training mode.
- Mostrare che in eval mode il dropout si spegne.
- Controllare che output head e token embedding condividano gli stessi pesi.
"""
...
from studio.snapshot.lezione_35.batching import create_batch
from studio.snapshot.lezione_35.generate import generate_text_from_checkpoint
from studio.snapshot.lezione_35.model import LanguageModel
from studio.snapshot.lezione_35.tokenizer import create_vocabulary, encode
from studio.snapshot.lezione_35.training import configure_optimizer, train_model
...
def main():
    ...
    print("Differenza tra due forward in training mode:")
    print(round(train_difference, 6))
    print()

    print("Differenza tra due forward in eval mode:")
