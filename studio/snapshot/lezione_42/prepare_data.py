...
- Prima il dataset veniva letto da un singolo file di testo già presente.
- Qui prepariamo FineWeb-Edu in streaming e salviamo token BPE in file binari.

Scopo del file:
- Scaricare solo la quantità scelta di FineWeb-Edu senza duplicare raw text.
- Tokenizzare con GPT-2 BPE tramite `tiktoken`.
- Scrivere `train.bin`, `val.bin` e `meta.json` in una cartella dati comune.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepara FineWeb-Edu in train.bin e val.bin con GPT-2 BPE.",
    )
...
def open_output_files(output_dir, overwrite):
    ...
    if existing and not overwrite:
        ...
        raise FileExistsError(
            f"File già presenti in {output_dir}: {existing_names}. "
            "Usa --overwrite per ricrearli."
        )
...
def prepare_dataset(args):
    if args.target_gb <= 0:
        raise ValueError("--target-gb deve essere maggiore di 0.")

    if not 0 <= args.validation_ratio < 1:
        raise ValueError("--validation-ratio deve essere compreso tra 0 e 1.")

    if args.progress_mb < 1:
        raise ValueError("--progress-mb deve essere almeno 1.")

    if args.max_documents is not None and args.max_documents < 1:
        raise ValueError("--max-documents deve essere almeno 1 quando è impostato.")
